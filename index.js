    ...
    (err,result)=> {
        if(err){
            ...
            let info =transporter.sendMail({
                from:'"Agro_shop"<sayedgouse1995@gmail.com>',
                to:username,
...
app.post("/addproduct",upload.single("file"),(req,res)=> {
    ...
    console.log(req.file)
    const sql="insert into agro_products(category,product_name,uom,qty,price,image,description) values(?,?,?,?,?,?,?)";
        dbcon.query(sql,[category,product_name,uom,qty,price,filename,description],(err,result)=>{
            if(err){
                console.log(err);}
            else{
                res.send(result);}
...
app.get('/productview',(req,res) => {
    dbcon.query("select * from agro_products",(err,result) => {
        if(err){
        console.log(err);}
        else{
            res.send(result);}
...
app.post('/sendorder/:id',(req,res) => {
    ...
    id=req.body.id
    uid=req.body.uid
    const q="select * from agro_products where id=?";
...
app.get('/myorder/:id',(req,res) => {
    ...
    const q="select a.id,a.pid,a.user_id,a.qty,a.price,a.total,a.order_date,a.order_time,a.order_status,a.payment_status,b.product_name from customer_orders as a join agro_products as b on a.user_id=? and a.pid=b.id";
...
app.post('/paybill/:id',(req,res) => {
    ...
    dbcon.query(q,[status,id],(err,result) => {
        ...
        console.log(err);}
            ...
            dbcon.query("insert into payment(user_id,payment_date,order_id,paid_amount,transaction_no)values(?,?,?,?,?)",
            [uid,cdate,id,price,payment_id])
            //console.log(result[0].price)
            res.send(result);
        }
    });
...
app.get('/cat_wise_menu/:cat',(req,res) => {
    ...
    const q="select * from agro_products where category=?";
...
app.get('/allorders/',(req,res) => {
    const uid=req.params.id;
    const q="select a.id,a.pid,a.user_id,a.qty,a.price,a.total,a.order_date,a.order_time,a.order_status,a.payment_status,b.product_name from customer_orders as a join agro_products as b on a.pid=b.id";
    dbcon.query(q,(err,result) => {
        if(err){
        console.log(err);}
        else{
